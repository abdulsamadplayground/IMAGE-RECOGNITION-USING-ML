import os
from cvzone.HandTrackingModule import HandDetector #Detects hand landmarks from the video feed.

import cv2  # used for computer vision tasks such as video capture 
...
def video_camera():  # A function to initialize and handle the video capturing process, along with hand gesture detection for menu selection.
    ...
    modePositions = [(1136, 196), (1000, 384), (1136, 581)] # #Coordinates to draw the selection visualizations.
    counterPause = 0
    selectionList = [-1, -1, -1]

    while True:
        success, img = cap.read() #Reads a frame from the video feed.
        ...
        if hands and counterPause == 0 and modeType < 3:
            ...
            if counter > 0:
                ...
                cv2.ellipse(imgBackground, modePositions[selection - 1], (103, 103), 0, 0,
                            counter * selectionSpeed, (0, 255, 0), 20) ##- Draws an animated ellipse around the selected mode while holding the gesture.
        ...
        key = cv2.waitKey(1)
        if key == ord('e'): # if e key pressed, call the man function 
